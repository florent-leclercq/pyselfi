#!/usr/bin/env python
#-------------------------------------------------------------------------------------
# pySELFI v1.0 -- pyselfi/power_spectrum/selfi.py
...
class power_spectrum_selfi(selfi):
    """Main class for power spectrum inference with SELFI.
    Child of the main class of the SELFI code
    """
    ...
    def logprior_hyperparameters(self,theta_norm,theta_norm_mean,theta_norm_std,k_corr,k_corr_mean,k_corr_std):
        ...
        Returns
        -------
        logpdf (double) : log pdf of the Gaussian prior at the evaluation point
    ...
    def loglikelihood_hyperparams(self,theta_fiducial,Nbin_opt_min,Nbin_opt_max,theta_norm,k_corr):
        """Returns the log pdf of the likelihood for hyperparameters theta_norm and k_corr
        See equation (27) in Leclercq. et al 2019
        ...
        Returns
        -------
        logpdf (double) : log pdf of the likelihood at the evaluation point
    ...
    def logposterior_hyperparameters(self,theta_fiducial,Nbin_opt_min,Nbin_opt_max,theta_norm,k_corr,theta_norm_mean=0.2,theta_norm_std=0.3,k_corr_mean=0.025,k_corr_std=0.015):
        """Returns the log pdf of the unnormalized posterior for hyperparameters theta_norm and k_corr
        (logprior_hyperparameters + loglikelihood_hyperparams)
        ...
        theta_norm (double) : theta_norm value where to evaluate the likelihood
        k_corr (double) : k_corr value where to evaluate the likelihood
        theta_norm_mean (optional, double, default=0.2) : mean of the Gaussian prior on theta_norm
        theta_norm_std (optional, double, default=0.3) : standard deviations of the Gaussian prior on theta_norm
        k_corr_mean (optional, double, default=0.025) : mean of the Gaussian prior on k_corr
        k_corr_std (optional, double, default=0.015) : standard deviations of the Gaussian prior on k_corr

        Returns
        -------
        logpdf (double) : log pdf of the unnormalized posterior at the evaluation point

        
        """
        return self.loglikelihood_hyperparams(theta_fiducial,Nbin_opt_min,Nbin_opt_max,theta_norm,k_corr)+self.logprior_hyperparameters(theta_norm,theta_norm_mean,theta_norm_std,k_corr,k_corr_mean,k_corr_std)
    ...
    def optimize_prior(self, theta_fiducial, k_opt_min, k_opt_max, x0=None,
        ...
        """Optimizes the SELFI prior
        See section II.E. in Leclercq et al. 2019
        ...
        x0 (optional, array, double, dimension=2, default=(prior.theta_norm,prior.k_corr)) : starting point in parameter space
        theta_norm_min (optional, double, default=1e-5) : boundary in parameter space
        theta_norm_max (optional, double, default=5.) : boundary in parameter space
        theta_norm_mean (optional, double, default=0.2) : mean of the Gaussian prior on theta_norm
        theta_norm_std (optional, double, default=0.3) : standard deviations of the Gaussian prior on theta_norm
        k_corr_min (optional, double, default=0.005) : boundary in parameter space
        k_corr_max (optional, double, default=0.05) : boundary in parameter space
        k_corr_mean (optional, double, default=0.025) : mean of the Gaussian prior on k_corr
        k_corr_std (optional, double, default=0.015) : standard deviations of the Gaussian prior on k_corr
        method (optional, string, default='L-BFGS-B') : optimization method. See documentation of scipy.optimize.minimize
        options (optional, dictionary, default={'maxiter':50, 'ftol':1e-20, 'gtol':1e-20, 'eps':1e-6, 'disp':True}) : optimization options. See documentation of scipy.optimize.minimize
        ...
        res = minimize(potential, x0, method=method,
               options=options, bounds=[[theta_norm_min,theta_norm_max],[k_corr_min,k_corr_max]])
        print(res)
    ...
    def restrict_posterior(self,Nbin_min,Nbin_max):
        ...
        theta_icov (array, double, dimension=(S',S')) : restricted inverse posterior covariance
        where S'=Nbin_max-Nbin_min
